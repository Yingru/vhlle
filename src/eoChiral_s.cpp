...
// ---- auxiliary EoS class. Two instances (objects) of this class will be created
...
class EoSaux{
  ...
  public:
    ...
    void gete(double s, double& e, double nb);
...
mustab = new double* [ne] ;
  ...
  for(int ie=0; ie<ne; ie++){
    //T [MeV], mu_q [MeV], energy density [e_0], pressure [e_0], baryon density [n_0], entropy density [n_0], mu_S [MeV], placeholder (not important).
    fin >> Ttab[ie][in] >> mubtab[ie][in] >> e[ie] >> ptab[ie][in] >> n[in] >> stab[ie][in] >> mustab[ie][in] >> a ;
  ...
  for (int ie = 0; ie < ne; ie++) etab[ie] = e[ie] * 0.146;
  for (int in = 0; in < nn; in++) ntab[in] = n[in] * 0.15;
  emin = e[0]*0.146 ;
  emax = e[ne-1]*0.146 ;
  nmin = n[0]*0.15 ;
  nmax = n[nn-1]*0.15 ;
  cout<<"EoSaux: table "<<filename<<" read, [emin,emax,nmin,nmax] = "<<emin<<"  "<<emax<<"  "<<nmin<<"  "<<nmax<<endl ;
  delete [] e ;
  delete [] n ;
}
...
void EoSaux::get(double e, double nb, double& p, double& T, double& mub, double &mus)
{
	if(e<0.) { T = mub = mus = p = 0. ; return ; }
...
void EoSaux::gete(double s, double& e, double nb)
{
  double mindist = 1.0e6;
////YX  for (int in = 0; in < nn; in++) {
    for (int ie = 0; ie < ne; ie++) {
      double sdist = fabs(s - stab[ie][0]);
      if (sdist < mindist) {
	mindist = sdist;
	e = etab[ie];
	nb = ntab[0];
...
void EoSChiral::eos(double e, double nb, double nq, double ns,
		double &T, double &mub, double &muq, double &mus, double &p)
{
  if(e<1.46 && nb<0.3) eossmall->get(e, nb, p, T, mub, mus) ;
  else eosbig->get(e, nb, p, T, mub, mus) ;
...
{
  if(e<1.46 && nb<0.3) return eossmall->p(e, nb) ;
  else return eosbig->p(e, nb) ;
}

void EoSChiral::gete(double s, double& e, double nb)
{
  eosbig->gete(s, e, nb) ;
  if (e<1.46 && nb<0.3) eossmall->gete(s, e, nb) ;
}
